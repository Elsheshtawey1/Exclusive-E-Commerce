...
let cartShopIcon = document.querySelector(".cart-shop");
...
cartShopIcon.addEventListener("click", openCart);
...
function getCartItems() {
  ...
  for (let i = 0; i < product_cart.length; i++) {
    ...
    total_price += parseFloat(product_cart[i].price);
  }
  items_in_cart.innerHTML = items_c;
  cartCount.innerHTML = product_cart.length;
  total.innerHTML = "$" + total_price.toFixed(2);
}
function remove_items(index) {
  product_cart.splice(index, 1);
  localStorage.setItem("product_cart", JSON.stringify(product_cart));
  getCartItems();
  let add_to_cart = document.querySelectorAll(".fa-cart-plus");
  add_to_cart.forEach((btn, idx) => {
